...
function InfantryPortal:OnInitialized()
    ...
    elseif Client then
        ...
        self.preventSpinDuration = 0

        self:AddFieldWatcher("queuedPlayerId", InfantryPortal.OnUpdateClientSpinEffect)
...
if Server then
    ...
    function InfantryPortal:FillQueueIfFree()
        ...
        if gameInfo and not gameInfo:GetWarmUpActive() and GetIsUnitActive(self) then
...
if Client then
    ...
    function InfantryPortal:PreventSpinEffect(duration)
        self.preventSpinDuration = Shared.GetTime() + duration
        DestroySpinEffect(self)
    end

    function InfantryPortal:OnUpdateClientSpinEffect()

        local shouldSpin = GetIsUnitActive(self) and self.queuedPlayerId ~= Entity.invalidId and self.preventSpinDuration < Shared.GetTime()
